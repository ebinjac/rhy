...
import { AlignLeft, BookOpen, Braces, Check, CircleAlert, FileWarning, LoaderCircle, Map, Network, Play, ScrollText, ShieldCheck, Sparkles, Variable } from "lucide-react"
...
export function PreRequestScriptEditor({value,onChange,monitorId,revisionId,stepId,request}:Props){
  const [mounted,setMounted]=useState(false);const [desktop,setDesktop]=useState(true);const [dark,setDark]=useState(false);const [running,setRunning]=useState(false);const [minimap,setMinimap]=useState(false);const [result,setResult]=useState<ScriptResultContract|null>(null);const [problems,setProblems]=useState<ScriptProblemContract[]>([]);const [message,setMessage]=useState("");const editorRef=useRef<Parameters<OnMount>[0]|null>(null);const monacoRef=useRef<Parameters<OnMount>[1]|null>(null)
  useEffect(()=>{setMounted(true);const media=window.matchMedia("(min-width: 768px)");const update=()=>setDesktop(media.matches);const updateTheme=()=>setDark(document.documentElement.classList.contains("dark"));const observer=new MutationObserver(updateTheme);update();updateTheme();media.addEventListener("change",update);observer.observe(document.documentElement,{attributes:true,attributeFilter:["class"]});return()=>{media.removeEventListener("change",update);observer.disconnect()}},[])
  ...
  return <div className="overflow-hidden rounded-xl border bg-background">
    ...
    <div className="relative min-h-[360px] bg-muted/25">{mounted&&desktop?<Suspense fallback={<EditorLoading/>}><MonacoEditor height="420px" language="javascript" theme={dark?"vs-dark":"light"} value={value.code} onChange={(code)=>change(code??"")} onMount={mount} options={{fontSize:13,lineHeight:21,fontLigatures:true,minimap:{enabled:minimap},automaticLayout:true,scrollBeyondLastLine:false,wordWrap:"on",padding:{top:14,bottom:14},tabSize:2,formatOnPaste:true,quickSuggestions:true,accessibilitySupport:"auto"}}/></Suspense>:<div className="p-3"><p className="mb-2 text-xs text-muted-foreground">Compact editor · use a desktop browser for autocomplete and advanced navigation.</p><Textarea className="min-h-[330px] resize-y bg-background font-mono text-[13px] leading-5 text-foreground" spellCheck={false} value={value.code} onChange={(event)=>change(event.target.value)} aria-label="JavaScript pre-request code"/></div>}</div>
...
function EvidencePanel({result,problems,message}:{result:ScriptResultContract|null;problems:ScriptProblemContract[];message:string}){
  const aux=result?.auxiliaryRequests??[]
  return <div className="min-h-40 resize-y overflow-auto border-t"><Tabs defaultValue="console" className="gap-0"><div className="border-b px-3"><TabsList variant="line"><TabsTrigger value="console"><ScrollText/> Console {result?.logs.length?`(${result.logs.length})`:""}</TabsTrigger><TabsTrigger value="requests"><Network/> pm.sendRequest {aux.length?`(${aux.length})`:""}</TabsTrigger><TabsTrigger value="variables"><Variable/> Variables</TabsTrigger><TabsTrigger value="changes"><Sparkles/> Request changes</TabsTrigger><TabsTrigger value="tests"><Check/> Tests</TabsTrigger><TabsTrigger value="problems"><CircleAlert/> Problems {problems.length?`(${problems.length})`:""}</TabsTrigger></TabsList></div><div className="max-h-64 overflow-auto p-3 font-mono text-xs">
  ...
  <TabsContent value="requests">{aux.length?aux.map((request,index)=>{const ok=request.success!==false&&!request.error;return <div className="space-y-1 border-b py-2 last:border-0" key={`${request.method}-${request.url}-${index}`}><div className="flex flex-wrap items-center gap-2 font-sans text-xs"><Badge variant="outline">Pre-request</Badge><span className={ok?"text-success":"text-destructive"}>{ok?"✓":"×"}</span><span className="font-mono font-medium">{request.method||"GET"}</span><span className="min-w-0 flex-1 truncate font-mono text-muted-foreground" title={request.url}>{request.url||"—"}</span><span className="font-mono">{request.status?`HTTP ${request.status}`:request.error?"No response":"—"}</span><span className="font-mono text-muted-foreground">{request.durationMs??0} ms</span></div>{request.error?<p className="font-sans text-xs text-destructive">{request.error}</p>:null}<p className="font-sans text-[11px] text-muted-foreground">pm.sendRequest #{index+1}</p></div>}):<Empty text="Outbound pm.sendRequest calls appear here after preview."/>}</TabsContent>
