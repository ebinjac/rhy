...
export const Route = createFileRoute("/monitors/$monitorId/metrics")({
  ...
  loader: async ({ params, deps }) => {
    const [runs, metrics] = await Promise.all([
      ...
      getMonitorMetrics({
        data: { monitorId: params.monitorId, window: deps.window },
      }),
    ])
    return { runs, metrics }
  },
        ...
        <div className="mt-5 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              Target response time, tail latency, reliability, spikes, and
              Rhythm execution overhead across this monitor&apos;s history.
            </p>
          </div>
