  ...
  return <div className="mx-auto max-w-[1280px] px-4 py-6 md:px-6 md:py-8">
    ...
    <section className="mt-8"><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end"><div><h2 className="font-heading text-lg font-semibold">Latest executions</h2><p className="mt-0.5 text-sm text-muted-foreground">Newest first. API response excludes Rhythm orchestration and checks.</p></div><Link className="inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:underline" params={{ monitorId }} to="/monitors/$monitorId/metrics">View performance metrics <ArrowRight className="size-3.5" /></Link></div>
      <div className="mt-4 flex flex-wrap gap-2 border-y py-3">
        ...
        <select aria-label="Filter run status" className="h-9 rounded-lg border bg-background px-3 text-sm" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1) }}><option value="">All statuses</option>{Array.from(new Set(runs.map((run) => run.status))).map((value) => <option key={value}>{value}</option>)}</select>
...
function RunRow({ run, compare, onCompare }: { run: RunContract; compare: boolean; onCompare: (checked: boolean) => void }) {
  ...
  const apiResponse = run.steps?.reduce((sum, step) => sum + Number(step.timing?.apiResponseMs ?? step.timing?.networkTotalMs ?? 0), 0)
  return <div className="grid gap-3 border-b px-4 py-4 last:border-b-0 hover:bg-muted/30 md:grid-cols-[32px_minmax(180px,1fr)_145px_130px_120px_120px_36px] md:items-center md:gap-4">
    ...
    <span className="text-sm text-muted-foreground">{run.triggerType.toLowerCase().replaceAll("_", " ")}</span><span className="font-mono text-sm">{active ? "In progress" : apiResponse ? formatDuration(apiResponse) : "Not recorded"}</span><span className="font-mono text-sm">{active ? "In progress" : formatDuration(run.durationMs)}</span><Link aria-label={`View diagnostics for run ${run.id}`} className="rounded focus-visible:ring-2 focus-visible:ring-ring" params={{ monitorId: run.monitorId, runId: run.id }} to="/monitors/$monitorId/runs/$runId"><ArrowRight className="size-4 text-muted-foreground" /></Link>
  </div>
}
